{%- macro generate_delete_query(sources) -%}
    {%- if sources is none -%}
        {{ exceptions.raise.compiler_error("!!!Error!!! sources can not be empty") }}
    {%- endif -%}

    {%- set ns = namespace(query = 'false', source_tables_region = [], source_tables_non_region = []) -%}
    {%- set ecc_regions = ['SAP_ECC_MANA','SAP_ECC_MXOT'] -%}

    {# separate sources into two categories - Tables that are available in 7 regions and tables available in single regions #}
    ...
    {# create CTE for source tables to filter out delete records #}

    {%- for i in ns.source_table_non_region -%}
        {%- set i = i.replace(' ','') -%}
        ...
        {%- set ns.query = ns.query + db_name + "_" + table_name + " as (select * from " + var_val + " where RECORDSTATUS != 'D' ), \n" -%}
    {%- endfor -%}
    ...
    {%- for region in ecc_regions -%}
        ...
        {%- set ns.query = ns.query + region + "_" + table_name + " as (select * from " + var_val + " where RECORDSTATUS != 'D' ), \n" -%}
    {%- endfor -%}
    {%- endfor -%}
    {{- ns.query -}}
{%- endmacro -%
